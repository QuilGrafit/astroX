...
# --- Render-specific imports and settings ---
from aiohttp import web # Already present, good.


# --- Загрузка переменных окружения для локальной разработки ---
load_dotenv()

# Настройки (теперь читаются из переменных окружения для Vercel/Render)
...
# [NEW] Add a secret key for cron job endpoint
CRON_SECRET_KEY = os.getenv("CRON_SECRET_KEY") # <-- НОВАЯ ПЕРЕМЕННАЯ ОКРУЖЕНИЯ
...
# [NEW] Check for CRON_SECRET_KEY
if not CRON_SECRET_KEY:
    logging.warning("Environment variable CRON_SECRET_KEY is not set. Cron job endpoint will be unprotected.")
...
# --- Ваши существующие классы и функции (Form, TEXTS, HoroscopeGenerator, Keyboard) без изменений ---
# ... (весь код от Form до начала обработчиков)
...
# --- Вспомогательная функция для определения языка (можно удалить, если всегда ру) ---
...
# Ежедневная рассылка гороскопов (вызывается извне Render Cron Job)
...
# --- [NEW] HTTP endpoint for cron-job.org ---
async def cron_job_handler(request: web.Request):
    """
    HTTP-обработчик для запуска ежедневной рассылки гороскопов.
    Проверяет секретный ключ в заголовке авторизации.
    """
    # Check for the secret key in the Authorization header
    auth_header = request.headers.get('Authorization')
    if not auth_header or not auth_header.startswith('Bearer '):
        logger.warning("Unauthorized access attempt to cron endpoint: Missing Bearer token.")
        return web.Response(text="Unauthorized: Missing Bearer token", status=401)
    
    received_key = auth_header.split(' ')[1]

    if received_key != CRON_SECRET_KEY:
        logger.warning(f"Unauthorized access attempt to cron endpoint: Invalid secret key. Received: {received_key}")
        return web.Response(text="Unauthorized: Invalid secret key", status=401)
    
    logger.info("Cron job endpoint accessed successfully. Running scheduled tasks...")
    try:
        await scheduled_tasks()
        logger.info("Scheduled tasks completed via cron endpoint.")
        return web.Response(text="Scheduled tasks executed successfully!", status=200)
    except Exception as e:
        logger.error(f"Error during scheduled tasks execution via cron endpoint: {e}", exc_info=True)
        return web.Response(text=f"Error executing scheduled tasks: {e}", status=500)


# Функция для установки вебхука и инициализации БД (будет вызвана при деплое на Vercel/Render)
async def on_startup(passed_bot: Bot) -> None:
    logger.info("Инициализация...")
    await init_mongodb()
    logger.info("Установка вебхука...")
    if not WEBHOOK_URL:
        logger.error("WEBHOOK_URL is not set. Cannot set webhook. Make sure WEBHOOK_HOST and PORT environment variables are configured correctly on Render.")
        return
    
    await passed_bot.set_webhook(WEBHOOK_URL, drop_pending_updates=True)
    logger.info(f"Вебхук установлен на: {WEBHOOK_URL}")

# Закрытие соединения с БД при завершении (для локальной разработки и Render)
async def on_shutdown(passed_bot: Bot) -> None:
    if mongo_client:
        mongo_client.close()
        logger.info("MongoDB соединение закрыто.")
    await passed_bot.delete_webhook()
...
async def main():
    ...
    if WEBHOOK_HOST and WEB_SERVER_PORT:
        logger.info(f"Запуск бота в режиме вебхука на {WEB_SERVER_PORT} (для Render).")
        web_app = web.Application()
        web_app.add_routes([web.post(WEBHOOK_PATH, dp.web_hook_update)])
        # [NEW] Add the cron job endpoint route
        web_app.add_routes([web.get('/run_daily_horoscopes', cron_job_handler)]) # Or any other path you prefer
        ...
        logger.error("Environment variables WEBHOOK_HOST or PORT are not set. Cannot run in webhook mode.")
        logger.info("Запуск бота в режиме long-polling (для локальной разработки, если WEBHOOK_HOST не задан).")
